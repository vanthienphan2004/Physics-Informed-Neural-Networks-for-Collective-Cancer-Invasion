...
class Visualizer:
    ...
    def plot_density_snapshot(self, time_index: Optional[int] = None) -> None:
        ...
        fig.suptitle(
            f"Cancer Invasion: Density Snapshot at t = {time_value:.2f}", fontsize=16
        )
    ...
    def plot_loss_comparison(
        ...
        plt.plot(
            epochs,
            self.loss_history["physics"],
            "b-",
            linewidth=2,
            label="PDE Physics Loss",
            alpha=0.8,
        )
        plt.plot(
            epochs,
            self.loss_history["ic"],
            "r-",
            linewidth=2,
        ...
        plt.title(
            "Training Loss Comparison - PINN Cancer Invasion",
            fontsize=16,
            fontweight="bold",
        )
    ...
    def save_loss_data(self, save_path: str) -> None:
        ...
        for loss_type, losses in self.loss_history.items():
            data[loss_type] = losses + [np.nan] * (
                max_length - len(losses)
    ...
    def visualize_all(self) -> None:
        ...
        if self.loss_history is not None:
            self.plot_loss_comparison(
                save_path=os.path.join(plot_dir, "training_losses.png"), show_plot=False
            )
